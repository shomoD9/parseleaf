...
Ran the release again and found that the Homebrew verification logic was now succeeding at the important part but failing during cleanup. `brew install` and `brew test parseleaf` both passed, but `brew untap local/parseleaf-verify` failed because Homebrew refuses to untap a tap that still owns an installed formula. Removed that teardown step from `.github/workflows/release.yml` and updated `ARCHITECTURE.md` so the release narrative is honest about the verification stopping after successful install-and-test rather than performing a forced untap.

Rewrote `README.md` so it now reads like a proper open-source project front page instead of an internal note. The new README leads with a clear product claim, explains why Parseleaf exists, gives direct install instructions for npm and Homebrew, shows a quick-start flow and example output layout, describes the current CLI scope, and includes a short development and release section. Updated `ARCHITECTURE.md` so the repository geography reflects that `README.md` is now a real public entrypoint rather than a sparse project note.

Revised `README.md` again after feedback that the first rewrite still sounded unnatural and over-explanatory. Tightened the language, removed phrases that read like tutorial scaffolding, and aimed the tone more squarely at what people expect from a normal open-source project page. Updated `ARCHITECTURE.md` so its description of the README matches that calmer, less didactic style.
...
Prepared the follow-up npm release so the rewritten README can actually appear on the npm package page. Bumped `package.json` and `package-lock.json` from `0.1.0` to `0.1.1`, and updated `ARCHITECTURE.md` to make explicit that npm shows the README that shipped with the published version rather than the current state of `main`.

Ran `npm run pack:check` again at `0.1.1` and confirmed that the publishable tarball is still clean, now with the larger rewritten README included. Attempted a direct local `npm publish --provenance=false --access public --cache /tmp/parseleaf-npm-cache` so the new README could reach npm immediately, but the command stalled after reaching the registry publish step in a way that suggested an interactive account approval path that could not be completed from this session. Decided to continue with the release-driven path instead of leaving the version bump half-finished.
